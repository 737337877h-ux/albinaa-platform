...
def import_file(conn, path, parse_result=None):
    ...
    for (code, ccy_raw), acc in res.accounts.items():
        ...
        snapshot_bal = acc.declared_balance if acc.declared_balance is not None else bal
        cur.execute("""INSERT INTO balance_snapshots(customer_id, currency_code, balance, import_job_id)
                       VALUES (?,?,?,?)""", (customer_id, acc.currency, snapshot_bal, job_id))
...
def verify_balances(res):
    """مطابقة الرصيد المحسوب مع الرصيد المعلن للحسابات أحادية الكتلة فقط.
    الحسابات متعددة الكتل (fragments > 1) مستبعدة لأن الرصيد المعلن
    فيها تراكمي بينما المحسوب يعتمد على الافتتاحي فقط."""
    checked = matched = mismatched = no_declared = fragmented = 0
    mismatches = []
    fragmented_accounts = []
    for acc in res.accounts.values():
        ...
        if acc.fragments > 1:
            fragmented += 1
            fragmented_accounts.append((
                acc.customer_code, acc.currency, acc.fragments,
    ...
    return {
        ...
        'no_declared_line': no_declared, 'fragmented_skipped': fragmented,
        'mismatch_details': mismatches, 'fragmented_details': fragmented_accounts,
    }
